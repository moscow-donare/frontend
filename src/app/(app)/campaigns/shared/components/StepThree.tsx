...
export const StepThree: React.FC = () => {
  const { formData, isCreateMode, isEditMode } = useCampaignFormContext();
  const { handlePrevStep, handleSubmit, isSubmitting } = useCampaignForm();
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-6 text-gray-900">
        {isCreateMode() ? 'Revisar y Confirmar' : 'Revisar Cambios y Confirmar'}
      </h2>
      ...
      <div className="mb-6 p-4 bg-warning-50 border border-warning-200 rounded-md flex items-start">
        ...
        <div>
          ...
          <p className="text-sm text-warning-700 mt-1">
            {isCreateMode() 
              ? 'Al crear esta campaña, aceptas que todos los fondos recaudados y transacciones serán registrados en la blockchain y serán completamente transparentes para los donantes. Asegúrate de que toda la información proporcionada sea correcta.'
              : 'Al actualizar esta campaña, los cambios serán registrados en la blockchain y serán visibles para todos los donantes. Asegúrate de que toda la información sea correcta antes de confirmar los cambios.'
            }
          </p>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit}>
        <div className="flex justify-between">
          ...
          <Button
            ...
            startContent={!isSubmitting ? (isCreateMode() ? <Upload className="h-4 w-4" /> : <Save className="h-4 w-4" />) : null}
          >
            {isSubmitting 
              ? (isCreateMode() ? 'Creando campaña...' : 'Guardando cambios...') 
              : (isCreateMode() ? 'Crear Campaña' : 'Guardar Cambios')
            }
          </Button>
        </div>
      </form>
