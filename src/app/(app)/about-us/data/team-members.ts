import augusto from "@public/images/about-us/augusto.jpeg"; 
import tomas from "@public/images/about-us/tomas.jpeg"; 
import nicolas from "@public/images/about-us/nicolas.jpeg"; 
import luis from "@public/images/about-us/luis.jpeg"; 
import stefano from "@public/images/about-us/stefano.jpeg"; 

export const TEAM_MEMBERS = [
  {
    name: "Tomás Villarreal",
    role: "Ingenieria en Sistemas",
    bio: "Especialista en desarrollo frontend con 5 años de experiencia en proyectos de impacto social.",
    image: tomas,
    linkedin: "https://www.linkedin.com/in/tomas-villarreal/",
  },
  {
    name: "Augusto Falco",
    role: "Ingenieria en Sistemas",
    bio: "Apasionado por crear experiencias digitales accesibles e inclusivas para todos los usuarios.",
    image: augusto,
    linkedin: "https://www.linkedin.com/in/augustofalco/",
  },
  {
    name: "Nicolas Ludueña",
    role: "Ingenieria en Sistemas",
    bio: "Coordinadora de iniciativas sociales con enfoque en sostenibilidad y desarrollo comunitario.",
    image: nicolas,
    linkedin: "https://www.linkedin.com/in/nicolasluduena/",
  },
  {
    name: "Luis Stoller",
    role: "Ingenieria en Sistemas",
    bio: "Especialista en arquitectura de sistemas y bases de datos para proyectos de gran escala.",
    image: luis,
    linkedin: "https://www.linkedin.com/in/luis-stoller/",
  },
  {
    name: "Stefano Bortolin",
    role: "Ingenieria en Sistemas",
    bio: "Estratega de comunicación con experiencia en campañas de impacto social y recaudación de fondos.",
    image: stefano,
    linkedin: "https://www.linkedin.com/in/stefano-bortolin-132b76256/",
  },
]